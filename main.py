...
@app.post("/api/transfers/enrich-ids")
async def api_transfers_enrich_ids():
    """Phase 2: usa os tags do LLM para buscar af_player_id e af_team_from_id via API-Football."""
    from transfer_enricher import enrich_all_ids
    result = await enrich_all_ids()
    return result
...
<form method="get">
  ...
  <button type="submit">Set ID</button>
    ...
    with get_conn() as conn:
        ...
        updated = c.rowcount
    return HTMLResponse(
        f"<pre>OK: {updated} registros: player_name like '{player_name}' -> {af_player_id}</pre>"
    )
